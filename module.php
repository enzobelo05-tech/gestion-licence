...
$requete = $connexion->query(
    "SELECT name, hours_count, id, parent_id, position
     FROM module
     ORDER BY COALESCE(parent_id, id), CASE WHEN parent_id IS NULL THEN 0 ELSE 1 END, COALESCE(position, id), id"
);
$tousLesModules = $requete->fetchAll(PDO::FETCH_ASSOC);

function comparerModules(array $gauche, array $droite): int
{
    $positionGauche = $gauche['position'] === null ? (int) $gauche['id'] : (int) $gauche['position'];
    $positionDroite = $droite['position'] === null ? (int) $droite['id'] : (int) $droite['position'];

    if ($positionGauche === $positionDroite) {
        return (int) $gauche['id'] <=> (int) $droite['id'];
    }

    return $positionGauche <=> $positionDroite;
}
